...
namespace CS2_SimpleAdmin
{
    public partial class CS2_SimpleAdmin
    {
        ...
        [RequiresPermissions("@css/permban")] // Ban other players
        ...
        internal void Ban(CCSPlayerController? caller, CCSPlayerController? player, int time, string reason, string? callerName = null, BanManager? banManager = null, CommandInfo? command = null)
        {
            ...
            if (time == 0)
            {
                if (!player.IsBot && !player.IsHLTV)
                    ...
                    {
                        player.PrintToCenter(_localizer!["sb_player_ban_message_perm", reason, caller == null ? "Console" : caller.PlayerName]);
                    }
                ...
                if (caller == null || !SilentPlayers.Contains(caller.Slot))
                {
                    foreach (var controller in Helper.GetValidPlayers())
                    {
                        using (new WithTemporaryCulture(controller.GetLanguage()))
                        {
                            ...
                            sb.Append(_localizer["sb_admin_ban_message_perm", callerName, player.PlayerName, reason]);
            ...
            else
            {
                if (!player.IsBot && !player.IsHLTV)
                    using (new WithTemporaryCulture(player.GetLanguage()))
                    {
                        player.PrintToCenter(_localizer!["sb_player_ban_message_time", reason, time, caller == null ? "Console" : caller.PlayerName]);
                    }
                if (caller == null || !SilentPlayers.Contains(caller.Slot))
                {
                    foreach (var controller in Helper.GetValidPlayers())
                    {
                        ...
                        {
                            StringBuilder sb = new(_localizer!["sb_prefix"]);
                            sb.Append(_localizer["sb_admin_ban_message_time", callerName, player.PlayerName, reason, time]);
                            controller.PrintToChat(sb.ToString());
                        }
                    }
                }
        ...
        private bool CheckValidBan(CCSPlayerController? caller, int duration)
        {
            ...
            bool canPermBan = AdminManager.PlayerHasPermissions(caller, "@css/permban"); // Ban other players

            if (duration == 0 && canPermBan == false)
            {
                caller.PrintToChat($"{_localizer!["sb_prefix"]} {_localizer["sb_ban_perm_restricted"]}");
                return false;
            }

            if (duration > Config.MaxBanDuration && canPermBan == false)
            {
                caller.PrintToChat($"{_localizer!["sb_prefix"]} {_localizer["sb_ban_max_duration_exceeded", Config.MaxBanDuration]}");
                return false;
            }
        ...
        [RequiresPermissions("@css/permban")] // Remove bans
